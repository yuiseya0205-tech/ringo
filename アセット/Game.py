...
LANG = {
    ...
    "enter_name":     {"ja": "Nキーで名前入力", "en": "Press N to enter name"},
...
cv2.namedWindow("ringokyatti", cv2.WINDOW_NORMAL)
cv2.resizeWindow("ringokyatti", 1280, 720)
cv2.setMouseCallback("ringokyatti", on_mouse)
...
while True:
    ...
    if mode == "count":
        sec = 2 - int(time.time() - count_start)
        if sec <= 0:
            if seated_mode:
